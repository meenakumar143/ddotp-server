...
app.get("/dashboard", noCache, async (req, res) => {
  try {
    ...
    <body>
      ...
      <input id="stockyard" value="${(user.stockyard || "").replace(/"/g, "&quot;")}">

      <label>Vehicle</label>
      <input id="vehicle" value="${(user.vehicle || "").replace(/"/g, "&quot;")}">

      <label>Delivery Code</label>
      <input id="delivery_code" value="${(user.delivery_code || "").replace(/"/g, "&quot;")}">
      ...
      async function loadData(){
        try{
          const res = await fetch(window.location.origin + "/secure-data?token=" + encodeURIComponent(token), {
            method: "GET",
            cache: "no-store",
            headers: {
...
// ===== Secure Data =====
app.get("/secure-data", noCache, async (req, res) => {
  try {
    const token = String(req.query.token || "").trim();
    const user = await User.findOne({ token });
...
// ===== Users =====
app.get("/admin/users", noCache, async (req, res) => {
  try {
...
// ===== Plain API =====
app.get("/api/user/stockyard/:username", noCache, async (req, res) => {
  try {
